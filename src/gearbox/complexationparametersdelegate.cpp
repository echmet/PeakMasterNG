...
QWidget *ComplexationParametersDelegate::createEditor(QWidget *parent, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
  ...
  const QVariant itemType = index.model()->data(index, ComplexationParameterTypeRole);
...
#else
  const QStringList blocks =  lineEdit->text().split(ComplexationRelationshipsModel::ELEMENTS_SPLITTER, Qt::SkipEmptyParts);
#endif // QT_VERSION
