                                                                 ...
                                                                 const QAbstractTableModel * const ezDetailsModelBase, double kBGE)
{
  ...
  for (int idx = 0; idx < ezDetailsModel->columnCount(); idx++) {
    ...
    AnalytesExtraInfoModel::ExtraInfo::Initializer init;

    init.uEff = ezProps.mobility;
    init.uEMD = ezProps.uEMD;
    init.kappaBGE = kBGE;
    ...
    if (detected)
    {
      init.time = tdInfo.timeMax;
      init.cMax = tdInfo.concentrationMax;
      init.cndMax = tdInfo.conductivityMax;
    }

    data.append({tdInfo.name, std::move(init)});
  }
...
void PMNGMainWindow::connectOnScreenChanged()
{
  ...
  m_mainCtrlWidget->connectOnScreenChanged();
}
